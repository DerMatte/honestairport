---
iata: "PKX"
name: "Beijing Daxing International Airport"
city: "Beijing"
country: "China"
lastUpdated: "2026-06-20"
sources:
  - "https://www.daxingairport.com/"
  - "https://www.caac.gov.cn/"
quickFacts:
  - "Opened September 2019, star-shaped terminal by Zaha Hadid"
  - "Primary hub for China Eastern, Beijing Capital, and China Southern"
  - "Daxing Airport Express reaches Caoqiao in ~19-22 min; Beijing South needs metro transfers (~35-46 min)"
  - "Three runways; designed for 72 million passengers annually"
  - "Domestic and international in single connected terminal"
bentoTips:
  - category: "timing"
    label: "Timing"
    title: "Arrive 3 hours early"
    summary: "Security and immigration queues lengthen sharply after 10 a.m."
    detail: "Domestic flights can clear in 90 minutes off-peak; international rarely under 2.5 hours."
  - category: "terminal"
    label: "Terminal"
    title: "Use the central atrium as reference"
    summary: "All gates radiate from the massive central hall; follow color-coded floor stripes."
    detail: "Walking times exceed 15 minutes to outer gates; moving walkways run only on the outer ring."
  - category: "food"
    label: "Food"
    title: "Head to Level 4 for local chains"
    summary: "Skip overpriced Level 2 outlets; Level 4 has Beijing duck and noodle stalls at normal prices."
    detail: "Open 06:30–21:00; seating fills after 18:00 on domestic departures."
  - category: "status"
    label: "Status"
    title: "Check PKX app or WeChat mini-program"
    summary: "Real-time gate and security wait times update every 5 minutes."
    detail: "Official sources beat airline apps when weather or slot restrictions hit."
lounges:
  - name: "Air China Lounge"
    terminal: "Main Terminal"
    zone: "domestic/international"
    access:
      - "day pass"
      - "membership"
    bestFor:
      - "Work"
    verdict: "depends"
    summary: "Paid day-pass option near gates 1-20 suits Air China flyers but offers limited upside for ordinary travelers."
  - name: "China Eastern Lounge"
    terminal: "Main Terminal"
    zone: "domestic/international"
    access:
      - "day pass"
      - "membership"
    bestFor:
      - "Work"
    verdict: "depends"
    summary: "Airside paid lounge near gates 1-20 works only if you already hold membership or need quick paid access."
---

# PKX Airport Guide

> One terminal, fast rail link, and strict security make advance planning essential.

## Quick Facts
- Single massive terminal handles all flights; no inter-terminal transfers needed.
- Daxing Airport Express runs every 10–15 minutes to Caoqiao (~19–22 min); Beijing South requires metro transfers (~35–46 min total).
- China Eastern and Beijing Capital base most operations here; Air China uses PEK.
- Free Wi-Fi requires Chinese phone number verification; VPN recommended for foreign SIMs.
- Taxis and ride-hailing only from designated Level 1 curb; no street hails.
- **Wi‑Fi:** Free airport Wi‑Fi — Chinese phone number verification required; VPN recommended for foreign SIMs.
- **Power:** Charging tables in central hall near information desks; scarce at outer gates.
- **Water:** Refill stations near every restroom cluster — bring an empty bottle.

## Security & Screening Tips
- Expect full liquid screening in clear bags; no PreCheck equivalent exists.
- Remove laptops and power banks at every checkpoint; bins move slowly.
- Peak congestion hits 09:00–11:00 and 17:00–20:00; arrive before or after these windows.
- International departures require both security and immigration; allow 40 extra minutes.

## Best Airport Tricks & Hacks
- Download the “Beijing Daxing Airport” WeChat mini-program before arrival for live queue times.
- Use the central atrium escalators to reach Level 4 food court quickly from any gate.
- High-speed rail tickets can be bought with Alipay or WeChat Pay at machines; cash not accepted.
- If your flight is delayed, ride the Airport Express to Caoqiao for better food and return on a later train.
- Power outlets are scarce at gates; charging tables sit only in the central hall near information desks.
- Avoid taxi touts inside arrivals; official dispatch is on Level 1 and issues printed receipts.
- Keep your passport and boarding pass together; staff scan both at multiple points.

## Terminals & Navigation
- One terminal with five concourses; color-coded floor lines (red, blue, green) lead to gates.
- Central security checkpoint splits into domestic and international streams; follow overhead signs.
- Minimum connection time posted at 60 minutes domestic-to-domestic; 90 minutes international.
- Moving walkways stop at 23:00; plan extra walking time on late arrivals.

## Lounges, Food & Amenities
- Level 4 offers the best value: Quanjude duck, Lanzhou noodles, and local coffee at street prices.
- Paid lounges (Air China, China Eastern) sit airside near gates 1–20; access via day pass or membership.
- Quiet seating areas with charging stations are located between concourses C and E.
- Free water refill stations exist near every restroom cluster.

## Ground Transport & Parking
- Daxing Airport Express to Caoqiao is fastest (~19–22 min); Beijing South needs metro transfers (~35–46 min total).
- Official taxis charge fixed airport rates; Didi ride-hailing requires Chinese app registration.
- Short-term parking (P1–P3) fills by 09:00; long-term lots require shuttle bus.
- Metro Line 19 connects but adds 50 minutes versus rail; use only if heading to southwest Beijing.

## Official Sources
- https://www.daxingairport.com/
- https://www.caac.gov.cn/ (flight status and regulations)
- Beijing Subway and high-speed rail schedules via official apps.
