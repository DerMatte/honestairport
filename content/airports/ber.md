...
- Lufthansa Group Fast Lane (Miles & More Frequent Traveller/Senator/HON plus LH Group business/first) uses a dedicated lane; on busy weekday mornings it can beat general CT queues, but still backs up when the airport opens extra Prio lanes dynamically — check live wait times.
...
- **BER Runway ended 26 May 2026:** Slot booking is discontinued — pick the shortest CT lane on terminal monitors instead. During holiday peaks BER has claimed sub-10-minute CT waits; verify on live displays before committing to a queue.
...
- Showers cost **€10** (towel and shampoo included; card payment only) on Terminal 1 arrivals level E0 ([BER showers](https://ber.berlin-airport.de/en/cafes-shops-service/sgs-poi/510-duschen-t1.html)). Lounge shower kits are a separate **€15** fee.

## Ground Transport & Parking
- **FEX** airport express is fastest to Hauptbahnhof (**~23 min** since Dec 2025 rail upgrades; **€5.00** ABC single ticket, no surcharge) — runs every **15 min** daytime ([VBB BER](https://www.vbb.de/en/driving-information/ber-airport)).
- S-Bahn line S9 uses the same **ABC €5.00** ticket and runs every **10 min** off-peak.
