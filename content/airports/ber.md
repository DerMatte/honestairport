...
- Lufthansa Group Fast Lane (Miles & More Frequent Traveller/Senator/HON plus LH Group business/first) uses a dedicated lane; compare live wait times against standard CT lanes — dynamic Prio lanes can shorten either queue on busy mornings.
...
- **BER Runway ended 26 May 2026** — use live CT wait times on the BER app instead of booking a security slot; on quiet weekdays CT lanes can show waits under 10 minutes (verify on monitors before joining any queue).
...
- Showers cost **€10** (towel and shampoo included, **card only**) on arrivals level E0 ([BER showers](https://ber.berlin-airport.de/en/cafes-shops-service/sgs-poi/510-duschen-t1.html)).

## Ground Transport & Parking
- **FEX** airport express is fastest to Hauptbahnhof (**~23 min**, **€5.00** ABC ticket) every **15 min** ([BER public transport](https://ber.berlin-airport.de/en/orientierung/getting-here/public-transport.html)).
- **S-Bahn S9** uses the same **ABC €5.00** ticket and runs every **10 min** — slower than FEX but more frequent off-peak.
