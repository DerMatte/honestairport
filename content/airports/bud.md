---
iata: "BUD"
name: "Budapest Ferenc Liszt International Airport"
city: "Budapest"
country: "Hungary"
lastUpdated: "2026-06-20"
sources:
  - "https://www.bud.hu/"
  - "https://www.bud.hu/en/passengers/security"
quickFacts:
  - "Terminals 2A (Schengen) and 2B (non-Schengen) linked by 5-minute walkway"
  - "Primary base for Wizz Air and Ryanair"
  - "Bus 100E reaches Deák Ferenc tér in 35 minutes for 2500 HUF"
  - "Security averages 20 minutes; morning peaks hit 45+"
  - "Schengen MCT after shared security: 30 min floor; 2A↔2B adds 5 min walk"
  - "Free Wi-Fi (BUD Free); power tables between security and gates"
bentoTips:
  - category: "timing"
    label: "Timing"
    title: "Arrive 90 minutes early"
    summary: "Skip the 3-hour recommendation unless checking bags on Ryanair or Wizz."
    detail: "Morning 6-9am and Friday evenings are the only times lines exceed 30 minutes."
  - category: "terminal"
    label: "Terminal"
    title: "Confirm terminal on boarding pass"
    summary: "2A and 2B share one security but split after; wrong side means backtrack."
    detail: "Non-Schengen flights almost always use 2B; the walkway is clearly signed."
  - category: "food"
    label: "Food"
    title: "Hit the langos stand post-security"
    summary: "Fresh Hungarian fried dough for €4-5 beats all chain options."
    detail: "Located in 2B departures near gate B12; open until last flight."
  - category: "status"
    label: "Status"
    title: "Watch the FIDS boards"
    summary: "Gates flip frequently due to apron works; check every 20 minutes."
    detail: "The official app and boards update faster than airline push notifications."
lounges:
  - name: "Skycourt Lounge"
    terminal: "2"
    access:
      - "Priority Pass"
      - "day pass"
    amenities:
      - "showers"
    bestFor:
      - "Rest"
    verdict: "depends"
    summary: "Average pay-per-use lounge worth visiting only if you specifically need a shower."
  - name: "Terminal 2A Lounge"
    terminal: "2A"
    access:
      - "Priority Pass"
      - "day pass"
    amenities:
      - "showers"
    bestFor:
      - "Rest"
    verdict: "skip"
    summary: "Skip this average pay-per-use lounge unless showers are essential."
  - name: "Terminal 2B Lounge"
    terminal: "2B"
    access:
      - "Priority Pass"
      - "day pass"
    amenities:
      - "showers"
    bestFor:
      - "Rest"
    verdict: "skip"
    summary: "Skip this average pay-per-use lounge unless showers are essential."
---

# BUD Airport Guide

> Fast, no-nonsense reference for BUD’s two-terminal layout, low-cost carrier crowds, and direct city links.

## Quick Facts
- Terminals 2A (Schengen) and 2B (non-Schengen) connected by indoor walkway.
- Wizz Air and Ryanair dominate; Lufthansa and Turkish also operate daily.
- Single security checkpoint serves both terminals.
- **MCT:** Same ticket, Schengen–Schengen after the shared checkpoint — **30 min** floor (SAS publishes this for European routes). Non-Schengen in 2B adds passport control time.
- 100E airport bus is fastest public option to central Budapest.
- **Wi‑Fi:** Free **BUD Free** network; login via browser. Power tables line the windows between security and gates.
- **Water:** Reliable fountains post-security near gate B12 — fill before boarding.

## Security & Screening Tips
- Online check-in and bag drop cut wait times dramatically for Wizz and Ryanair.
- Liquids in 100 ml containers only; no PreCheck equivalent.
- Peak congestion 05:30-09:00 and 17:00-20:00; aim outside those windows.
- Shoes stay on; laptops and large electronics must come out.

## Best Airport Tricks & Hacks
- Download the bud.hu app for live gate changes and 100E bus times.
- Fill water bottles after security near gate B12; fountains are reliable.
- Use the quiet seating area opposite the langos stand when gates are crowded.
- Buy 100E tickets from the machine outside arrivals to skip driver cash hassle.
- If your flight is delayed, the 2B observation deck offers free views and seating.
- Keep a screenshot of your boarding pass; WiFi occasionally drops during boarding.
- Taxi stands are regulated; never accept rides from inside the terminal.

## Terminals & Navigation
- 2A and 2B share one security hall; after screening, signs direct you left (2A) or right (2B).
- Walking time between farthest gates is under 12 minutes.
- Common mistake: following airline staff to the wrong side of security when gates change.

## Lounges, Food & Amenities
- Pay-per-use lounges in both terminals are average; skip unless you need showers.
- Langos stand in 2B and the sausage grill in 2A are the only local food worth buying.
- Free charging tables line the windows between security and gates.

## Ground Transport & Parking
- 100E bus (2500 HUF, every 15-20 min) beats taxis in traffic and cost.
- Official taxi rank uses fixed fares; Budapest city center is €25-30.
- Short-term parking (€3/hour) sits directly outside departures; long-term lots require a free shuttle.
- Train to Nyugati station exists but requires a bus transfer and is rarely faster than 100E.

## Official Sources
- https://www.bud.hu/
- https://www.bud.hu/en/passengers/security
- https://bkk.hu/en/ (for 100E and public transport)
