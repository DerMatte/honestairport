...
/**
 * Horizontal photo strip for an airport, fed by the image sync pipeline
 * (Wikimedia Commons → Vercel Blob). Renders nothing while an airport has
 * no photos yet — the pipeline backfills airports one by one via cron.
 * Credit + license are shown per image (CC attribution requirement).
 */
export async function AirportPhotoGallery({ iata }: { iata: string }) {
  ...
  return (
    <section aria-label={`${iata} photos`}>
